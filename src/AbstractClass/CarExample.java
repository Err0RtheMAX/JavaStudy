...
class Tire {
    ...
    public boolean roll() {
        ...
        if (accumulatedRotation < maxRotation) { // 최대한 달릴 수 있는 횟수가 소비 횟수보다 작으면 남은 수명 출력하기
            System.out.println(location + "AbstractClass.Tire 수명 : " + (maxRotation - accumulatedRotation) + " 회");
            return true;

        } else {
            System.out.println("***" + location + " AbstractClass.Tire 펑크 *** ");
            return false;
        }
    }
}
...
class Car{
    // 생성자를 AbstractClass.Tire 클래스를 이용하여 생성하기
...
class HankookTire extends Tire{
    ...
    public boolean roll(){
        ...
        if(accumulatedRotation < maxRotation){
            System.out.println(location + " AbstractClass.HankookTire 수명: " + (maxRotation - accumulatedRotation) + "회");
            return true;
        }

        else{
            System.out.println("*** " + location + " AbstractClass.HankookTire 펑크 ***");
            return false;
        }
    }
}
...
class KumhoTire extends Tire {
    ...
    public boolean roll() {
        ++accumulatedRotation;
        if (accumulatedRotation < maxRotation) {
            System.out.println(location + " AbstractClass.KumhoTire 수명 : " + (maxRotation - accumulatedRotation) + " 회");
            return true;
        } else {
            System.out.println("*** " + location + " AbstractClass.KumhoTire 펑크 ***");
            return false;
        }
    }
}
